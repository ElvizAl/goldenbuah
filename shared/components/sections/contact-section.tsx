...
export default function ContactSection() {
  return (
    <section id="kontak" className="py-12 sm:py-16 md:py-24 scroll-mt-20">
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="space-y-4">
              <ContactInfo
                icon={<MapPin className="h-5 w-5 text-green-500 mt-0.5 shrink-0" />}
                title="Alamat"
                details={["Jl. Kenaiban no. 54 karawaci tangerang"]}
              />
              <ContactInfo
                icon={<Phone className="h-5 w-5 text-green-500 mt-0.5 shrink-0" />}
                title="Telepon"
                details={["+62 812 3456 7890"]}
              />
              <ContactInfo
                icon={<Clock className="h-5 w-5 text-green-500 mt-0.5 shrink-0" />}
                title="Jam Buka"
                details={["Senin - Sabtu: 08.00 - 20.00", "Minggu: 09.00 - 18.00"]}
              />
            </div>
            <Button className="mt-6 bg-[#01BC1D] hover:bg-[#00a519] w-full sm:w-auto" size="lg">
              Hubungi Kami
            </Button>
          </div>
          <div id="maps" className="h-64 sm:h-80 md:h-96 rounded-2xl overflow-hidden border shadow-sm scroll-mt-24">
            <iframe
